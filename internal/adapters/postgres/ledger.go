...
var (
	...
	selectLedgerPage = `SELECT ` + columns(ledgerColumns) + ` FROM wagering.wallet_ledger_entry ` +
	...
	selectLedgerAll = `SELECT ` + columns(ledgerColumns) + ` FROM wagering.wallet_ledger_entry ` +
		`WHERE wallet_id = $1 ORDER BY wallet_version`

	insertLedgerEntry = insertInto("wagering.wallet_ledger_entry", ledgerColumns)
)
